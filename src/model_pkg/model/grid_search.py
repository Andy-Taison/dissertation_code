...
def create_grid() -> list[dict]:
    ...
    """
    print("Creating grid...")
    batch_sizes = [32]  #[32, 64]  # Possibly trial 16, and 128 later
    latent_dims = [2]  #[2, 4, 8]  # Possibly trial 16 later
    ...
    learning_rates = [1e-4] # [1e-4, 1e-3]  # Later potentially trial 5e-4, 5e-3, and 1e-2 for fine-tuning
    weight_decay = [0] #[0, 1e-4]  # Possibly later trial 1e-2
    betas = [1]  #[0.1, 1]  # Possibly trial 0.5, 2, and 4 later
...
def train_grid_search(train_ds: TensorDataset, val_ds: TensorDataset, model_architecture_name: str, clear_history_list: bool = True, history_list_filename: str = "grid_search_list"):
    """
    ...
    :param clear_history_list: When False, files already listed in '<history_list_filename>.txt' are skipped
    :param history_list_filename: Filename without extension for storing paths to history files
    """
    print("=" * 50)
    ...
    with search_list_path.open(mode) as file:
        ...
        for i, setup in enumerate(grid):
            ...
            if f"{model_name}_history.pth" in completed_histories:
                ...
                print(f"\nConfiguration: [{i:>4d}/{len(grid):>4d}]")
...
def perform_grid_search(history_list_filename: str = "grid_search_list", loss_f1_tradeoff: int = 0.7) -> TrainingHistory | None:
    """
    ...
    :return: TrainingHistory found with best score
    """
    print(">" * 50)
    ...
    for filename in history_filenames:
        ...
        score = loss_f1_tradeoff * history.bests['best_loss'] + (1 - loss_f1_tradeoff) * (1 - history.bests['best_f1_avg'])
    ...
    print(f"Best Configuration tradeoff score: {best_score:.4f}")
    print(best_history)
    print(">" * 50)
